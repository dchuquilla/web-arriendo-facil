...
(function () {
  ...
  ready(function init() {
    ...
    function setLoading(loading) {
      if (loadingEl) {
        if (loading) loadingEl.removeAttribute('hidden');
    ...
    function validateBeforeSubmit() {
      ...
      if (!rentalYears || rentalYears < 1 || rentalYears > 20) return 'El tiempo de arriendo debe estar entre 1 y 20 años.';
      if (phone && !/^\d{10}$/.test(phone)) return 'El teléfono debe tener exactamente 10 dígitos.';
      if (idNumber && !/^\d{10}$/.test(idNumber)) return 'La cédula debe tener exactamente 10 dígitos.';
      return '';
    }
    ...
    function bootstrapTokenValidation() {
      if (!selector || !token) {
        showStatus('Enlace inválido. Falta selector o token.', true);
